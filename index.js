...
async function registerSlashCommands(token, clientId) {
  if (!token) {
    return console.error('❌ TOKEN no encontrado en Render.');
  }
...
client.once('ready', async () => {
  ...
  await registerSlashCommands(process.env.TOKEN, client.user.id);
});
...
// ==========================================
// 7. PUENTE DE MENSAJES BIDIRECCIONAL (MD ↔ TICKET Y COMMAND !reply)
// ==========================================
